...
import com.wasu.videoplayer.model.Model;
import com.wasu.videoplayer.model.VideoOptionModel;
...
import com.wasu.videoplayer.utils.FileUtils;
...
public class VideoManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    public static synchronized VideoManager instance(Context context) {
    ...
    public static HttpProxyCacheServer getProxy(Context context) {
        HttpProxyCacheServer proxy = VideoManager.instance(context).proxy;
        return proxy == null ? (VideoManager.instance(context).proxy =
                VideoManager.instance(context).newProxy(context)) : proxy;
    }
    ...
    public static HttpProxyCacheServer getProxy(Context context, File file) {
        ...
        if (VideoManager.instance(context).cacheFile != null
                && !VideoManager.instance(context).cacheFile.getAbsolutePath().equals(file.getAbsolutePath())) {
            //不一致先关了旧的
            HttpProxyCacheServer proxy = VideoManager.instance(context).proxy;
            ...
            //开启新的
            return (VideoManager.instance(context).proxy =
                    VideoManager.instance(context).newProxy(context, file));
        } else {
            //还没有缓存文件的或者一致的，返回原来
            HttpProxyCacheServer proxy = VideoManager.instance(context).proxy;

            return proxy == null ? (VideoManager.instance(context).proxy =
                    VideoManager.instance(context).newProxy(context, file)) : proxy;
        }
    }
    ...
    @Override
    public void onVideoSizeChanged(IMediaPlayer mp, int i, int i1) {
    ...
    private void initIJKPlayer(Message msg) {
        ...
        try {
            if (VideoType.isMediaCodec()) {
                ...
                ((ffplay) mediaPlayer).setOption(ffplay.OPT_CATEGORY_PLAYER, "mediacodec", 1);
                ((ffplay) mediaPlayer).setOption(ffplay.OPT_CATEGORY_PLAYER, "mediacodec-auto-rotate", 1);
                ((ffplay) mediaPlayer).setOption(ffplay.OPT_CATEGORY_PLAYER, "mediacodec-handle-resolution-change", 1);
            }
            ((ffplay)mediaPlayer).setOption(1,"user-agent","wasutv_player1.1");
            ((ffplay) mediaPlayer).setDataSource(((Model) msg.obj).getUrl(), ((Model) msg.obj).getMapHeadData());
            mediaPlayer.setLooping(((Model) msg.obj).isLooping());
            if (((Model) msg.obj).getSpeed() != 1 && ((Model) msg.obj).getSpeed() > 0) {
                ((ffplay) mediaPlayer).setSpeed(((Model) msg.obj).getSpeed());
            }
            initIJKOption((ffplay) mediaPlayer);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void initEXOPlayer(Message msg) {
        mediaPlayer = new WasuExoPlayer(context);
        mediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        try {
    ...
    private void initIJKOption(ffplay ijkMediaPlayer) {
    ...
    private void showDisplay(Message msg) {
        if (msg.obj == null && mediaPlayer != null) {
            ...
            if (mediaPlayer instanceof WasuExoPlayer) {
     ...
     */
    public  void onPause() {
        if (VideoManager.instance(context).listener() != null) {
            VideoManager.instance(context).listener().onVideoPause();
        }
    }
    ...
    public  void onResume() {
        if (VideoManager.instance(context).listener() != null) {
            VideoManager.instance(context).listener().onVideoResume();
        }
    }
