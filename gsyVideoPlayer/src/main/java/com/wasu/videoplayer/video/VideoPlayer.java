...
import com.wasu.videoplayer.utils.StorageUtils;
...
public abstract class VideoPlayer extends BaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath, Object... objects) {
        ...
        if (cacheWithPlay && url.startsWith("http") && !url.contains("127.0.0.1")) {
            ...
            //注册上缓冲监听
            if (!mCacheFile && VideoManager.instance(getContext()) != null) {
                proxy.registerCacheListener(VideoManager.instance(getContext()), mOriginUrl);
            }
        }
        this.mUrl = url;
    ...
    protected void setStateAndUi(int state) {
        ...
        switch (mCurrentState) {
            case CURRENT_STATE_NORMAL:
                if (isCurrentMediaListener()) {
                    cancelProgressTimer();
                    VideoManager.instance(getContext()).releaseMediaPlayer();
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            ...
            if (mCurrentState == CURRENT_STATE_NORMAL || mCurrentState == CURRENT_STATE_ERROR) {
                ...
                VideoManager.instance(getContext()).getMediaPlayer().pause();
                ...
                VideoManager.instance(getContext()).getMediaPlayer().start();
    ...
    protected void prepareVideo() {
        if (VideoManager.instance(getContext()).listener() != null) {
            VideoManager.instance(getContext()).listener().onCompletion();
        }
        VideoManager.instance(getContext()).setListener(this);
        VideoManager.instance(getContext()).setPlayTag(mPlayTag);
        VideoManager.instance(getContext()).setPlayPosition(mPlayPosition);
        addTextureView();
        mAudioManager.requestAudioFocus(onAudioFocusChangeListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
        ((Activity) getContext()).getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        mBackUpPlayingBufferState = -1;
        VideoManager.instance(getContext()).prepare(mUrl, mMapHeadData, mLooping, mSpeed);
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                    if (VideoManager.instance(getContext()).getMediaPlayer().isPlaying()) {
                        VideoManager.instance(getContext()).getMediaPlayer().pause();
                    }
                    break;
    ...
    public void onVideoPause() {
        if (VideoManager.instance(getContext()).getMediaPlayer().isPlaying()) {
            ...
            mCurrentPosition = VideoManager.instance(getContext()).getMediaPlayer().getCurrentPosition();
    ...
    public void onVideoResume() {
        ...
        if (mCurrentState == CURRENT_STATE_PAUSE) {
            if (mCurrentPosition > 0 && VideoManager.instance(getContext()).getMediaPlayer() != null) {
                setStateAndUi(CURRENT_STATE_PLAYING);
                VideoManager.instance(getContext()).getMediaPlayer().seekTo(mCurrentPosition);
                VideoManager.instance(getContext()).getMediaPlayer().start();
            }
        }
    }
    ...
    protected void addTextureView() {
        ...
        if (mTextureView == null){
            mTextureView = new MyTextureView(getContext());
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        VideoManager.instance(getContext()).setDisplay(mSurface);
        //显示暂停切换显示的图片
        showPauseCover();
    }
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        VideoManager.instance(getContext()).setDisplay(null);
        surface.release();
        cancelProgressTimer();
        return true;
    }
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (VideoManager.instance(getContext()).getMediaPlayer() != null && mHadPlay) {
            int time = seekBar.getProgress() * getDuration() / 100;
            VideoManager.instance(getContext()).getMediaPlayer().seekTo(time);
        }
    }
    ...
    public void onPrepared() {
        ...
        if (VideoManager.instance(getContext()).getMediaPlayer() != null && mSeekToInAdvance != -1) {
            VideoManager.instance(getContext()).getMediaPlayer().seekTo(mSeekToInAdvance);
            mSeekToInAdvance = -1;
        }
        ...
        if (VideoManager.instance(getContext()).getMediaPlayer() != null && mSeekOnStart > 0) {
            VideoManager.instance(getContext()).getMediaPlayer().seekTo(mSeekOnStart);
            mSeekOnStart = 0;
        }
    ...
    public void onAutoCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {
            ...
            if (VideoManager.instance(getContext()).lastListener() != null) {
                VideoManager.instance(getContext()).lastListener().onAutoCompletion();
            }
        }
        if (!mIfCurrentIsFullscreen)
    ...
    public void onCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {//如果在进入全屏后播放完就初始化自己非全屏的控件
            IF_FULLSCREEN_FROM_NORMAL = false;
            if (VideoManager.instance(getContext()).lastListener() != null) {
                VideoManager.instance(getContext()).lastListener().onCompletion();//回到上面的onAutoCompletion
            }
        }
        if (!mIfCurrentIsFullscreen) {
            VideoManager.instance(getContext()).setListener(null);
            VideoManager.instance(getContext()).setLastListener(null);
        }
        VideoManager.instance(getContext()).setCurrentVideoHeight(0);
        VideoManager.instance(getContext()).setCurrentVideoWidth(0);
...
//                Debuger.printfLog("Net speed: " + getNetSpeedText() + " percent " + percent);
    ...
    public void onVideoSizeChanged() {
        int mVideoWidth = VideoManager.instance(getContext()).getCurrentVideoWidth();
        int mVideoHeight = VideoManager.instance(getContext()).getCurrentVideoHeight();
    ...
    public int getCurrentPositionWhenPlaying() {
        ...
        if (mCurrentState == CURRENT_STATE_PLAYING || mCurrentState == CURRENT_STATE_PAUSE) {
            try {
                position = (int) VideoManager.instance(getContext()).getMediaPlayer().getCurrentPosition();
    ...
    public int getDuration() {
        int duration = 0;
        try {
            duration = (int) VideoManager.instance(getContext()).getMediaPlayer().getDuration();
        } catch (IllegalStateException e) {
            e.printStackTrace();
            return duration;
        }
    ...
    protected boolean isCurrentMediaListener() {
        return VideoManager.instance(getContext()).listener() != null
                && VideoManager.instance(getContext()).listener() == this;
    }
    ...
    public boolean backFromWindowFull(Context context) {
        ...
        if (oldF != null) {
            ...
            hideNavKey(context);
            if (VideoManager.instance(getContext()).lastListener() != null) {
                VideoManager.instance(getContext()).lastListener().onBackFullscreen();
            }
        }
        return backFrom;
    }
...
//            return ((ffplay) VideoManager.instance(getContext()).getMediaPlayer()).getTcpSpeed();
//        } else {
//            return -1;
//        }
//
