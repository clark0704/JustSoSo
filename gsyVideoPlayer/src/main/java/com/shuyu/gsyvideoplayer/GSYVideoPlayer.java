...
public abstract class GSYVideoPlayer extends GSYBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath, Object... objects) {
        ...
        if (cacheWithPlay && url.startsWith("http") && !url.contains("127.0.0.1")) {
            ...
            //注册上缓冲监听
            if (!mCacheFile && GSYVideoManager.instance(getContext()) != null) {
                proxy.registerCacheListener(GSYVideoManager.instance(getContext()), mOriginUrl);
            }
        }
        this.mUrl = url;
    ...
    protected void setStateAndUi(int state) {
        ...
        switch (mCurrentState) {
            case CURRENT_STATE_NORMAL:
                if (isCurrentMediaListener()) {
                    cancelProgressTimer();
                    GSYVideoManager.instance(getContext()).releaseMediaPlayer();
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            ...
            if (mCurrentState == CURRENT_STATE_NORMAL || mCurrentState == CURRENT_STATE_ERROR) {
                ...
                GSYVideoManager.instance(getContext()).getMediaPlayer().pause();
                ...
                GSYVideoManager.instance(getContext()).getMediaPlayer().start();
    ...
    protected void prepareVideo() {
        if (GSYVideoManager.instance(getContext()).listener() != null) {
            GSYVideoManager.instance(getContext()).listener().onCompletion();
        }
        GSYVideoManager.instance(getContext()).setListener(this);
        GSYVideoManager.instance(getContext()).setPlayTag(mPlayTag);
        GSYVideoManager.instance(getContext()).setPlayPosition(mPlayPosition);
        addTextureView();
        mAudioManager.requestAudioFocus(onAudioFocusChangeListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
        ((Activity) getContext()).getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        mBackUpPlayingBufferState = -1;
        GSYVideoManager.instance(getContext()).prepare(mUrl, mMapHeadData, mLooping, mSpeed);
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                    if (GSYVideoManager.instance(getContext()).getMediaPlayer().isPlaying()) {
                        GSYVideoManager.instance(getContext()).getMediaPlayer().pause();
                    }
                    break;
    ...
    public void onVideoPause() {
        if (GSYVideoManager.instance(getContext()).getMediaPlayer().isPlaying()) {
            ...
            mCurrentPosition = GSYVideoManager.instance(getContext()).getMediaPlayer().getCurrentPosition();
            if (GSYVideoManager.instance(getContext()).getMediaPlayer() != null)
                GSYVideoManager.instance(getContext()).getMediaPlayer().pause();
        }
    }
    ...
    public void onVideoResume() {
        ...
        if (mCurrentState == CURRENT_STATE_PAUSE) {
            if (mCurrentPosition > 0 && GSYVideoManager.instance(getContext()).getMediaPlayer() != null) {
                setStateAndUi(CURRENT_STATE_PLAYING);
                GSYVideoManager.instance(getContext()).getMediaPlayer().seekTo(mCurrentPosition);
                GSYVideoManager.instance(getContext()).getMediaPlayer().start();
            }
        }
    }
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        GSYVideoManager.instance(getContext()).setDisplay(mSurface);
        //显示暂停切换显示的图片
        showPauseCover();
    }
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        GSYVideoManager.instance(getContext()).setDisplay(null);
        surface.release();
        cancelProgressTimer();
        return true;
    }
        ...
        if (id == R.id.surface_container) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (mChangePosition) {
                        GSYVideoManager.instance(getContext()).getMediaPlayer().seekTo(mSeekTimePosition);
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (GSYVideoManager.instance(getContext()).getMediaPlayer() != null && mHadPlay) {
            int time = seekBar.getProgress() * getDuration() / 100;
            GSYVideoManager.instance(getContext()).getMediaPlayer().seekTo(time);
        }
    }
    ...
    public void onPrepared() {
        ...
        if (GSYVideoManager.instance(getContext()).getMediaPlayer() != null && mSeekToInAdvance != -1) {
            GSYVideoManager.instance(getContext()).getMediaPlayer().seekTo(mSeekToInAdvance);
            mSeekToInAdvance = -1;
        }
        ...
        if (GSYVideoManager.instance(getContext()).getMediaPlayer() != null && mSeekOnStart > 0) {
            GSYVideoManager.instance(getContext()).getMediaPlayer().seekTo(mSeekOnStart);
            mSeekOnStart = 0;
        }
    ...
    public void onAutoCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {
            ...
            if (GSYVideoManager.instance(getContext()).lastListener() != null) {
                GSYVideoManager.instance(getContext()).lastListener().onAutoCompletion();
            }
        }
        if (!mIfCurrentIsFullscreen)
            GSYVideoManager.instance(getContext()).setLastListener(null);
        mAudioManager.abandonAudioFocus(onAudioFocusChangeListener);
    ...
    public void onCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {//如果在进入全屏后播放完就初始化自己非全屏的控件
            IF_FULLSCREEN_FROM_NORMAL = false;
            if (GSYVideoManager.instance(getContext()).lastListener() != null) {
                GSYVideoManager.instance(getContext()).lastListener().onCompletion();//回到上面的onAutoCompletion
            }
        }
        if (!mIfCurrentIsFullscreen) {
            GSYVideoManager.instance(getContext()).setListener(null);
            GSYVideoManager.instance(getContext()).setLastListener(null);
        }
        GSYVideoManager.instance(getContext()).setCurrentVideoHeight(0);
        GSYVideoManager.instance(getContext()).setCurrentVideoWidth(0);
...
//                Debuger.printfLog("Net speed: " + getNetSpeedText() + " percent " + percent);
    ...
    public void onVideoSizeChanged() {
        int mVideoWidth = GSYVideoManager.instance(getContext()).getCurrentVideoWidth();
        int mVideoHeight = GSYVideoManager.instance(getContext()).getCurrentVideoHeight();
    ...
    public int getCurrentPositionWhenPlaying() {
        ...
        if (mCurrentState == CURRENT_STATE_PLAYING || mCurrentState == CURRENT_STATE_PAUSE) {
            try {
                position = (int) GSYVideoManager.instance(getContext()).getMediaPlayer().getCurrentPosition();
    ...
    public int getDuration() {
        int duration = 0;
        try {
            duration = (int) GSYVideoManager.instance(getContext()).getMediaPlayer().getDuration();
        } catch (IllegalStateException e) {
            e.printStackTrace();
            return duration;
        }
    ...
    protected boolean isCurrentMediaListener() {
        return GSYVideoManager.instance(getContext()).listener() != null
                && GSYVideoManager.instance(getContext()).listener() == this;
    }
    ...
    public boolean backFromWindowFull(Context context) {
        ...
        if (oldF != null) {
            ...
            hideNavKey(context);
            if (GSYVideoManager.instance(getContext()).lastListener() != null) {
                GSYVideoManager.instance(getContext()).lastListener().onBackFullscreen();
            }
        }
        return backFrom;
    }
...
//        if (GSYVideoManager.instance(getContext()).getMediaPlayer() != null
//                && (GSYVideoManager.instance(getContext()).getMediaPlayer() instanceof ffplay)) {
//            return ((ffplay) GSYVideoManager.instance(getContext()).getMediaPlayer()).getTcpSpeed();
//        } else {
//            return -1;
//        }
//
