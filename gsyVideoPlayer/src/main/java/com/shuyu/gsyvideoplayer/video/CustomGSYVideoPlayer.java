...
public class CustomGSYVideoPlayer extends StandardGSYVideoPlayer {
    ...
    protected void addTextureView() {
        ...
        mPreviewTexture.setSurfaceTextureListener(new TextureView.SurfaceTextureListener() {
            ...
            public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
                GSYPreViewManager.instance(getContext()).setDisplay(new Surface(surface));
            }
            ...
            public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
                GSYPreViewManager.instance(getContext()).setDisplay(null);
                return true;
            }
    ...
    protected void prepareVideo() {
        GSYPreViewManager.instance(getContext()).prepare(mUrl, mMapHeadData, mLooping, mSpeed);
        super.prepareVideo();
    }
    ...
    public void onProgressChanged(SeekBar seekBar, final int progress, boolean fromUser) {
        ...
        if (fromUser && mOpenPreView) {
            ...
            if (GSYPreViewManager.instance(getContext()).getMediaPlayer() != null
                    && mHadPlay && (mOpenPreView)
                    && GSYPreViewManager.instance(getContext()).isSeekToComplete()) {
                GSYPreViewManager.instance(getContext()).setSeekToComplete(false);
                int time = progress * getDuration() / 100;
                GSYPreViewManager.instance(getContext()).getMediaPlayer().seekTo(time);
