...
public abstract class GSYBaseVideoPlayer extends FrameLayout implements GSYMediaPlayerListener {
    ...
    protected void resolveNormalVideoShow(View oldF, ViewGroup vp, GSYVideoPlayer gsyVideoPlayer) {
        ...
        mCurrentState = GSYVideoManager.instance(getContext()).getLastState();
        if (gsyVideoPlayer != null) {
            mCurrentState = gsyVideoPlayer.getCurrentState();
        }
        GSYVideoManager.instance(getContext()).setListener(GSYVideoManager.instance(getContext()).lastListener());
    ...
    public GSYBaseVideoPlayer startWindowFullscreen(final Context context, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            ...
            GSYVideoManager.instance(getContext()).setLastListener(this);
            GSYVideoManager.instance(getContext()).setListener(gsyVideoPlayer);
    ...
    public void setSpeed(float speed) {
        ...
        if ((GSYVideoManager.instance(getContext()).getMediaPlayer() != null)
            ...
            if (speed != 1 && speed > 0) {
                ((ffplay) GSYVideoManager.instance(getContext()).getMediaPlayer()).setSpeed(speed);
            }
        }
    }
