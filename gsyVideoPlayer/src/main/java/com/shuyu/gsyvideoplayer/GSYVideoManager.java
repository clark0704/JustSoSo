...
public class GSYVideoManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    public static synchronized GSYVideoManager instance(Context context) {
    ...
    public static HttpProxyCacheServer getProxy(Context context) {
        HttpProxyCacheServer proxy = GSYVideoManager.instance(context).proxy;
        return proxy == null ? (GSYVideoManager.instance(context).proxy =
                GSYVideoManager.instance(context).newProxy(context)) : proxy;
    }
    ...
    public static HttpProxyCacheServer getProxy(Context context, File file) {
        ...
        if (GSYVideoManager.instance(context).cacheFile != null
                && !GSYVideoManager.instance(context).cacheFile.getAbsolutePath().equals(file.getAbsolutePath())) {
            //不一致先关了旧的
            HttpProxyCacheServer proxy = GSYVideoManager.instance(context).proxy;
            ...
            //开启新的
            return (GSYVideoManager.instance(context).proxy =
                    GSYVideoManager.instance(context).newProxy(context, file));
        } else {
            //还没有缓存文件的或者一致的，返回原来
            HttpProxyCacheServer proxy = GSYVideoManager.instance(context).proxy;

            return proxy == null ? (GSYVideoManager.instance(context).proxy =
                    GSYVideoManager.instance(context).newProxy(context, file)) : proxy;
        }
    }
    ...
    private void initIJKPlayer(Message msg) {
        ...
        try {
            ...
            ((ffplay) mediaPlayer).setDataSource(((GSYModel) msg.obj).getUrl(), ((GSYModel) msg.obj).getMapHeadData());
            mediaPlayer.setLooping(((GSYModel) msg.obj).isLooping());
            if (((GSYModel) msg.obj).getSpeed() != 1 && ((GSYModel) msg.obj).getSpeed() > 0) {
                ((ffplay) mediaPlayer).setSpeed(((GSYModel) msg.obj).getSpeed());
            }
     ...
     */
    public  void onPause() {
        if (GSYVideoManager.instance(context).listener() != null) {
            GSYVideoManager.instance(context).listener().onVideoPause();
        }
    }
    ...
    public  void onResume() {
        if (GSYVideoManager.instance(context).listener() != null) {
            GSYVideoManager.instance(context).listener().onVideoResume();
        }
    }
