...
import static com.huwenmin.playerexample.utils.CommonUtil.getTextSpeed;
...
public abstract class WasuVideoPlayer extends WasuBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    protected void init(Context context) {
        ...
        mWasuVideoManager =WasuVideoManager.getInstance(mContext);
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath, Object... objects) {
        ...
        if (cacheWithPlay && url.startsWith("http") && !url.contains("127.0.0.1")) {
            HttpProxyCacheServer proxy = WasuVideoManager.getProxy(getContext().getApplicationContext(), cachePath);
            //此处转换了url，然后再赋值给mUrl。
            url = proxy.getProxyUrl(url);
            mCacheFile = (!url.startsWith("http"));
            //注册上缓冲监听
            if (!mCacheFile && WasuVideoManager.getInstance(mContext) != null) {
                proxy.registerCacheListener(WasuVideoManager.getInstance(mContext), mOriginUrl);
            }
        }
        this.mUrl = url;
    ...
    protected void setStateAndUi(int state) {
        ...
        switch (mCurrentState) {
            case CURRENT_STATE_NORMAL:
                if (isCurrentMediaListener()) {
                    cancelProgressTimer();
                    mWasuVideoManager.releaseMediaPlayer();
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            ...
            if (mCurrentState == CURRENT_STATE_NORMAL || mCurrentState == CURRENT_STATE_ERROR) {
                ...
                mWasuVideoManager.getMediaPlayer().pause();
                setStateAndUi(CURRENT_STATE_PAUSE);
                if (mVideoAllCallBack != null && isCurrentMediaListener()) {
                    if (mIfCurrentIsFullscreen) {
                        LogUtil.e("onClickStopFullscreen");
                        mVideoAllCallBack.onClickStopFullscreen(mUrl, mObjects);
                    } else {
                        LogUtil.e("onClickStop");
    ...
    protected void prepareVideo() {
        if (mWasuVideoManager.listener() != null) {
            mWasuVideoManager.listener().onCompletion();
        }
        mWasuVideoManager.setListener(this);
        mWasuVideoManager.setPlayTag(mPlayTag);
        mWasuVideoManager.setPlayPosition(mPlayPosition);
        addTextureView();
        mAudioManager.requestAudioFocus(onAudioFocusChangeListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
        ((Activity) getContext()).getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        mBackUpPlayingBufferState = -1;
        mWasuVideoManager.prepare(mUrl, mLooping, mSpeed);
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                    if (mWasuVideoManager.getMediaPlayer().isPlaying()) {
                        mWasuVideoManager.getMediaPlayer().pause();
                    }
                    break;
    ...
    public void onVideoPause() {
        if (mWasuVideoManager.getMediaPlayer().isPlaying()) {
            ...
            mCurrentPosition = mWasuVideoManager.getMediaPlayer().getCurrentPosition();
    ...
    public void onVideoResume() {
        ...
        if (mCurrentState == CURRENT_STATE_PAUSE) {
            if (mCurrentPosition > 0 && mWasuVideoManager.getMediaPlayer() != null) {
                setStateAndUi(CURRENT_STATE_PLAYING);
                mWasuVideoManager.getMediaPlayer().seekTo(mCurrentPosition);
                mWasuVideoManager.getMediaPlayer().start();
            }
        }
    }
    ...
    protected void addTextureView() {
        ...
        mTextureView = null;
        mTextureView = new WasuTextureView(getContext());
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        mWasuVideoManager.setDisplay(mSurface);
        //显示暂停切换显示的图片
        showPauseCover();
    }
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        mWasuVideoManager.setDisplay(null);
        surface.release();
        cancelProgressTimer();
        return true;
    }
        ...
        if (id == R.id.surface_container) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (mChangePosition) {
                        mWasuVideoManager.getMediaPlayer().seekTo(mSeekTimePosition);
                        int duration = getDuration();
                        int progress = mSeekTimePosition * 100 / (duration == 0 ? 1 : duration);
                        mProgressBar.setProgress(progress);
                        if (mVideoAllCallBack != null && isCurrentMediaListener()) {
                            LogUtil.e("onTouchScreenSeekPosition");
                            mVideoAllCallBack.onTouchScreenSeekPosition(mUrl, mObjects);
                        }
                    } else if (mBrightness) {
                        if (mVideoAllCallBack != null && isCurrentMediaListener()) {
                            LogUtil.e("onTouchScreenSeekLight");
                            mVideoAllCallBack.onTouchScreenSeekLight(mUrl, mObjects);
                        }
                    } else if (mChangeVolume) {
                        if (mVideoAllCallBack != null && isCurrentMediaListener()) {
                            LogUtil.e("onTouchScreenSeekVolume");
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        if (mVideoAllCallBack != null && isCurrentMediaListener()) {
            if (isIfCurrentIsFullscreen()) {
                LogUtil.e("onClickSeekbarFullscreen");
                mVideoAllCallBack.onClickSeekbarFullscreen(mUrl, mObjects);
            } else {
                LogUtil.e("onClickSeekbar");
                mVideoAllCallBack.onClickSeekbar(mUrl, mObjects);
            }
        }
        if (mWasuVideoManager.getMediaPlayer() != null && mHadPlay) {
            int time = seekBar.getProgress() * getDuration() / 100;
            mWasuVideoManager.getMediaPlayer().seekTo(time);
        }
    }
    ...
    public void onPrepared() {
        ...
        if (mWasuVideoManager.getMediaPlayer() != null && mSeekToInAdvance != -1) {
            mWasuVideoManager.getMediaPlayer().seekTo(mSeekToInAdvance);
            mSeekToInAdvance = -1;
        }
        ...
        if (mWasuVideoManager.getMediaPlayer() != null && mSeekOnStart > 0) {
            mWasuVideoManager.getMediaPlayer().seekTo(mSeekOnStart);
            mSeekOnStart = 0;
        }
    ...
    public void onAutoCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {
            ...
            if (mWasuVideoManager.lastListener() != null) {
                mWasuVideoManager.lastListener().onAutoCompletion();
            }
        }
        if (!mIfCurrentIsFullscreen)
            mWasuVideoManager.setLastListener(null);
        mAudioManager.abandonAudioFocus(onAudioFocusChangeListener);
    ...
    public void onCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {//如果在进入全屏后播放完就初始化自己非全屏的控件
            IF_FULLSCREEN_FROM_NORMAL = false;
            if (mWasuVideoManager.lastListener() != null) {
                mWasuVideoManager.lastListener().onCompletion();//回到上面的onAutoCompletion
            }
        }
        if (!mIfCurrentIsFullscreen) {
            mWasuVideoManager.setListener(null);
            mWasuVideoManager.setLastListener(null);
        }
        mWasuVideoManager.setCurrentVideoHeight(0);
        mWasuVideoManager.setCurrentVideoWidth(0);
    ...
    public void onBufferingUpdate(int percent) {
        if (mCurrentState != CURRENT_STATE_NORMAL && mCurrentState != CURRENT_STATE_PREPAREING) {
            if (percent != 0) {
                ...
                LogUtil.e("Net speed: " + getNetSpeedText() + " percent " + percent);
            }
            //循环清除进度
    ...
    public void onVideoSizeChanged() {
        int mVideoWidth = mWasuVideoManager.getCurrentVideoWidth();
        int mVideoHeight = mWasuVideoManager.getCurrentVideoHeight();
    ...
    public void clearCurrentCache() {
        if (mCacheFile) {
            //是否为缓存文件
            LogUtil.e(" mCacheFile Local Error " + mUrl);
            //可能是因为缓存文件除了问题
    ...
    private void deleteCacheFileWhenError() {
        clearCurrentCache();
        LogUtil.e("Link Or mCache Error, Please Try Again" + mUrl);
        mUrl = mOriginUrl;
    }
    ...
    public int getCurrentPositionWhenPlaying() {
        ...
        if (mCurrentState == CURRENT_STATE_PLAYING || mCurrentState == CURRENT_STATE_PAUSE) {
            try {
                position = (int) mWasuVideoManager.getMediaPlayer().getCurrentPosition();
    ...
    public int getDuration() {
        int duration = 0;
        try {
            duration = (int) mWasuVideoManager.getMediaPlayer().getDuration();
        } catch (IllegalStateException e) {
            e.printStackTrace();
            return duration;
        }
    ...
    protected boolean isCurrentMediaListener() {
        return mWasuVideoManager.listener() != null
                && mWasuVideoManager.listener() == this;
    }
    ...
    public  boolean backFromWindowFull(Context context) {
        ...
        if (oldF != null) {
            ...
            hideNavKey(context);
            if (mWasuVideoManager.lastListener() != null) {
                mWasuVideoManager.lastListener().onBackFullscreen();
            }
        }
        return backFrom;
    }
...
//            return ((ffplay)mWasuVideoManager.getMediaPlayer()).getTcpSpeed();
//        } else {
            return -1;
//        }
